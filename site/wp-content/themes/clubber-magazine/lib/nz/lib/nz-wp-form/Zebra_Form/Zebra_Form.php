            ...
            if ( $type == 'radios' || $type == 'checkboxes' ) {
                  ...
                  // if everything is ok
                        ...
                        foreach ( $values as $value => $caption ) {
                              ...
                              // create control
                              $obj = & $this->add(
                                                  ($type == 'radios' ? 'radio' : 'checkbox' ), $name, $value, (isset( $default ) && in_array( $value, $default ) ?
...
//                  $this->form_properties[ 'attributes' ][ 'class' ] = 'Zebra_Form';
            ...
            foreach ( $this->controls as $key => $control ) {
                  ...
                  if ( isset( $attributes[ 'type' ] ) && $attributes[ 'type' ] == 'select' && !isset( $attributes[ 'multiple' ] ) && isset( $attributes[ 'other' ] ) ) {
                        ...
                        $this->controls = array_slice( $this->controls, 0, $parent_position + 1, true ) +
            ...
            if ( $template == '' || $template == '*horizontal' || $template == '*vertical' ) {
                  ...
                  if ( $template == '*horizontal' ) {
                        ...
                        foreach ( $blocks as $controls ) {
                              ...
                              $class = '';
                              if ( isset( $controls[ 0 ] ) ) {//is option, select,
                                    if ( strpos( $controls[ 0 ], 'label_' ) === 0 ) {
                                          $class .= 'row-' . str_replace( 'label_', '', $controls[ 0 ] );
                                    } else {//btn_submit
                                          $class .= 'row-' . $controls[ 0 ];
                                    }
                              } else {
                              ...
                              $contents .= '<div class="row ' . $class . ( ++$counter % 2 == 0 ? ' even' : '') . ($counter == $rows ? ' last' : '') . '">';
                  ...
                  foreach ( $this->controls as $key => $control ) {
                        ...
                        if ( isset( $this->form_properties[ 'auto_fill' ][ 0 ][ $attributes[ 'name' ] ] ) ) {
                              ...
                              if ( $attributes[ 'type' ] == 'checkbox' || $attributes[ 'type' ] == 'radio' ) {
                                    ...
                                    $keys = array_slice(
                                              array_keys( $control->attributes[ 'options' ] ), (isset( $control->attributes[ 'multiple' ] ) && strtolower( trim( $control->attributes[ 'multiple' ] ) ) == 'multiple' ? 0 : 1 )
                                    );
                  ...
                  elseif ( !isset( $method[ $this->form_properties[ 'identifier' ] ] ) || $force === true ) {
                        ...
                        if ( $this->form_properties[ 'csrf_storage_method' ] == 'session' ) {
                              ...
                              // store the CSRF token in a cookie and use also the httponly argument
                                    if ( !setcookie(
                                                        $this->form_properties[ 'csrf_cookie_name' ], $this->form_properties[ 'csrf_token' ], $csrf_token_expiry, $this->form_properties[ 'csrf_cookie_config' ][ 'path' ], $this->form_properties[ 'csrf_cookie_config' ][ 'domain' ], $this->form_properties[ 'csrf_cookie_config' ][ 'secure' ], $this->form_properties[ 'csrf_cookie_config' ][ 'httponly' ]
                                              ) )
                                    ...
                                    if ( !setcookie(
                                                        $this->form_properties[ 'csrf_cookie_name' ], $this->form_properties[ 'csrf_token' ], $csrf_token_expiry, $this->form_properties[ 'csrf_cookie_config' ][ 'path' ], $this->form_properties[ 'csrf_cookie_config' ][ 'domain' ], $this->form_properties[ 'csrf_cookie_config' ][ 'secure' ]
                                              ) )
                                          trigger_error( 'The library tried to store the CSRF token in a cookie but was unable to do so because there was output already sent to the browser. You should either start a session prior to instantiating the library (recommended), have no output (including <html> and <head> tags, as well as any whitespace) sent to the browser prior to instantiating the library, or turn output buffering on in php.ini.', E_USER_ERROR );
                        }
                  }
                  ...
                  foreach ( $conditions as $proxy => $required_values ) {
                        ...
                        // if we don't have a cached result of the result
                              ...
                              if (
                                    ...
                                    if ( in_array( $this->controls[ $proxy ]->attributes[ 'type' ], array( 'image', 'submit' ) ) )
