...
abstract Class AjaxLogin {
       ...
       */
      static function status( $key = null, $code = null, $value = null ) {

            $status = array(
                  'invalid_credentials' => array(
                        'status' => 'fail',
                        'description' => __( 'Invalid credentials', 'ajax_login_register' ),
                  ),
                  'user_inactive' => array(
                        'status' => 'fail',
                        'description' => __( 'User inactive', 'ajax_login_register' ),
                  ),
                  'user_creation_error' => array(
                        'status' => 'fail',
                        'description' => __( 'Error creating user', 'ajax_login_register' ),
                  ),
                  'success_login' => array(
                  ...
                  'success_registration' => array(
                        ...
                        'description' => __( 'Registration complete', 'ajax_login_register' ),
                        'redirect' => get_author_posts_url( get_current_user_id() ),
                        'code' => $code
                  ),
            );
            ...
            if ( $value ) {
                  $status[ $key ][ 'type' ] = $value;
            }
            if ( $code ) {
                  $status[ $key ][ 'code' ] = $code;
            }

            return isset( $status[ $key ] ) ? $status[ $key ] : array( 'error' );
      }
