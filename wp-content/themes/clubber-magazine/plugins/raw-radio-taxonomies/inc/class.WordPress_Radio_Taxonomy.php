...
if (!class_exists('WordPress_Radio_Taxonomy')) :
      ...
      class WordPress_Radio_Taxonomy {
            ...
            public function metabox($post, $box) {
                  ...
                  <div id="taxonomy-<?php echo $taxonomy; ?>" class="radio-buttons-for-taxonomies">
                        ...
                        <div id="<?php echo $taxonomy; ?>-all" class="wp-tab-panel tabs-panel">
                              ...
                              ?>
                              <ul id="<?php echo $taxonomy; ?>checklist" data-wp-lists="list:<?php echo $taxonomy ?>" class="<?php
                        ...
                        <?php if (current_user_can($tax->cap->edit_terms)) : ?>
                              ...
                              ?>
                                                                        <input type="button" id="<?php echo $taxonomy; ?>-add-submit" data-wp-lists="add:<?php echo $taxonomy ?>checklist:<?php echo $taxonomy ?>-add" class="button <?php
                              if (is_taxonomy_hierarchical($taxonomy)) {
            ...
            function save_taxonomy_term($post_id) {
                  ...
                  if (is_array($this->tax_obj->object_type) && isset($_POST['post_type']) && !in_array($_POST['post_type'], $this->tax_obj->object_type))
                        return;

                  // verify this came from our plugin - one of our nonces must be set
                  if (!isset($_POST["_radio_nonce-{$this->taxonomy}"]) && !isset($_POST["_ajax_nonce-add-{$this->taxonomy}"]))
                        return;

                  // verify the nonce if this is an ajax "add term" action
                  if (isset($_POST["_ajax_nonce-add-{$this->taxonomy}"]) && !wp_verify_nonce($_POST["_ajax_nonce-add-{$this->taxonomy}"], "add-{$this->taxonomy}"))
                        return;

                  // verify the nonce if we're just saving the post normally
                  if (isset($_POST["_radio_nonce-{$this->taxonomy}"]) && !wp_verify_nonce($_POST["_radio_nonce-{$this->taxonomy}"], "radio_nonce-{$this->taxonomy}"))
                        return;
            ...
            function get_terms($terms, $taxonomies, $args) {
                  ...
                  if (!apply_filters('raw-radio-taxonomies-no-term-' . $this->taxonomy, TRUE))
                        return $terms;

                  //comment this to remove default selected empty item
                  /*
                    ...
                    $no_term = sprintf(__('No %s', 'raw-radio-taxonomies'), $this->tax_obj->labels->singular_name);
            ...
            function add_columns_init() {
                  ...
                  if (isset($this->tax_obj->object_type) && is_array($this->tax_obj->object_type))
                        foreach ($this->tax_obj->object_type as $post_type) {
                  ...
                  switch ($column) {
                        case "radio-{$this->taxonomy}":
                              if ($terms = wp_get_object_terms($post_id, $this->taxonomy)) {
                                    ...
                                    foreach ($terms as $t) {
                                          if (count($out) == 1)
                                                break; //break out of this foreach at 1 term only (is filtering get_the_terms better?
                                          ...
                                          $out[] = sprintf('<a href="%s">%s</a>', esc_url(add_query_arg($posts_in_term_qv, 'edit.php')), esc_html(sanitize_term_field('name', $t->name, $t->term_id, $this->taxonomy, 'display'))
                                          );
                                    ...
                                    printf(__('No %s', 'raw-radio-taxonomies'), $this->tax_obj->labels->singular_name);
                              }
                              break;
                  }
