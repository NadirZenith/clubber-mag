...
class Post_Type_Archive_Links {
      ...
      public function add_meta_box() {
            add_meta_box(
                    $this->metabox_id, __('Post Type Archives', 'hptal-textdomain'), array($this, 'metabox'), 'nav-menus', 'side', 'low'
            );
      }
      ...
      public function metabox_script($hook) {
            ...
            wp_register_script(
                    'hptal-ajax-script', CLUBBER_PLUGIN_URL . '/' . basename(__FILE__, '.php') . '/metabox.js', array('jquery'), filemtime(plugin_dir_path(__FILE__) . 'metabox.js'), true
                    /* 'hptal-ajax-script', plugins_url('metabox.js', __FILE__), array('jquery'), filemtime(plugin_dir_path(__FILE__) . 'metabox.js'), true */
            );
            wp_enqueue_script('hptal-ajax-script');

            // Add nonce variable
            wp_localize_script(
                    'hptal-ajax-script', 'hptal_obj', array(
      ...
      public function metabox() {
            ...
            foreach ($post_types as $pt) {
                  $html .= sprintf(
                          '<li><label><input type="checkbox" value ="%s" />&nbsp;%s</label></li>', esc_attr($pt->name), esc_attr($pt->labels->name)
                  );
            }
            $html .= '</ul>';
      ...
      public function maybe_make_current($items) {
            foreach ($items as $item) {
                  ...
                  if (
                          !is_post_type_archive($post_type) AND !is_singular($post_type)
                  )
                        continue;
      ...
      public function get_item_ancestors($item) {
            ...
            while (
            $anc_id = get_post_meta($anc_id, '_menu_item_menu_item_parent', true) AND !in_array($anc_id, $active_anc_item_ids)
            )
