...
if (!class_exists('WPMaintenanceMode')) {
      ...
      class WPMaintenanceMode {
            ...
            function add_scripts() {
                  ...
                  wp_register_script(
                          'jquery-ui-timepicker-addon', $this->get_plugins_url('js/jquery-ui-timepicker-addon.js', __FILE__), array('jquery-ui-datepicker'), '02-22-2013', TRUE
                  );

                  wp_register_script(
                          'wp-maintenance-mode', $this->get_plugins_url('js/wp-maintenance-mode.js', __FILE__), array('jquery-ui-datepicker', 'jquery-ui-timepicker-addon'), '', TRUE
                  );
                  wp_enqueue_script('jquery-ui-timepicker-addon');
                  wp_enqueue_script('wp-maintenance-mode');
                  // for nonce check on JS
                  wp_localize_script(
                          'wp-maintenance-mode', 'wp_maintenance_mode_vars', array(
            ...
            public function on_active() {
                  ...
                  if ((!$this->check_role() ) && !strstr($_SERVER['PHP_SELF'], 'wp-login.php') && !strstr($_SERVER['PHP_SELF'], 'wp-admin/') && !strstr($_SERVER['PHP_SELF'], 'async-upload.php') && !( strstr($_SERVER['PHP_SELF'], 'upgrade.php') && $this->check_role() ) && !strstr($_SERVER['PHP_SELF'], '/plugins/') && !strstr($_SERVER['PHP_SELF'], '/xmlrpc.php') && !$this->check_exclude() && !$this->check_bypass()
                  ) {
                        $rolestatus = 'norights';
            ...
            public function add_content() {
                  ...
                  if (isset($value['radio']) && 1 === $value['radio'] && 0 !== $td[2]) {
                        $echo = wp_sprintf(
                                stripslashes_deep($value['text']), '<br /><span id="countdown"></span>', date_i18n(get_option('date_format'), strtotime($td[0][0]))
                        );
