...
function menu_a_z() {
      ...
      <ul>
            ...
            for ($i = 0; $i < 26; ++$i) {
                  ...
                  ?>
                  <li>
                        <a style="float: left" href="<?php echo $letter_link; ?>" title="letter-<?php echo $this_letter; ?>"> [ <?php echo $this_letter; ?> ] </a>
                  </li>
                  <?php
            }
            ?>
...
function query_by_first_letter() {
      ...
      $sql = $wpdb->prepare("
                  ...
                  WHERE $wpdb->posts.post_type = 'artists'
...
function sort_all_by_first_letter() {
      ...
      if ($query->have_posts()) {
            ...
            <ul id="a-z" class="">
                  ...
                  while ($query->have_posts()) {
                        $query->the_post();
                        $first_letter = strtoupper(substr(apply_filters('the_title', get_the_title()), 0, 1));
                        /* d(get_the_title()); */
                        if ($first_letter != $curr_letter) {//IF NEW LETTER
                              if ($open_ul) {
                                    echo '</ul>';
                                    $open_ul = false;
                              }
                              ...
                              ?>
                              <lh style="">
                                    <h1 class="ml5"><?php echo $first_letter ?></h1>
                                    <hr class="pb5">
                              </lh>
