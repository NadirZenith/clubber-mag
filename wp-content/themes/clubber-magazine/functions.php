...
add_filter( 'wppb_login_redirect_message', 'nz_redirect_after_login' );

function nz_redirect_after_login( $this ) {
    global $wppb_login;
    $link = get_author_posts_url( $wppb_login->ID );
    $return = '<font id="messageTextColor">' . sprintf( __( 'You will soon be redirected automatically. If you see this page for more than 1 second, please click %1$s', 'profilebuilder' ), '<a href="' . $link . '">' . __( 'here', 'profilebuilder' ) . '</a>.<meta http-equiv="Refresh" content="1;url=' . $link . '" />' ) . '</font><br/><br/>';
    return $return;
}
...
function nz_remove_feeds() {
    remove_action( 'wp_head', 'feed_links_extra', 3 );
    remove_action( 'wp_head', 'feed_links', 2 );
}

/* add_action('after_setup_theme', 'nz_remove_feeds'); */
?>