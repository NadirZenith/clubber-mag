...
(function($) {
    ...
    sam.media = media = {
        ...
        handleMediaAttachment: function(a) {
            ...
            if ('' == $(this.adName).val() && '' != attachment.title)
                $(this.adName).val(attachment.title);
            if ('' == $(this.adDesc).val() && '' != attachment.caption)
                $(this.adDesc).val(attachment.caption);
            if ('' == $(this.adAlt).val() && '' != attachment.alt)
                $(this.adAlt).val(attachment.alt);
        }
    };
    ...
    $(document).ready(function() {
        ...
        fu = new AjaxUpload(btnUpload, {
            ...
            onComplete: function(file, response) {
                ...
                if (response == "success") {
                    ...
                    if (em == 'item')
                        $("#ad_img").val(samStrs.url + file);
                    else if (em == 'place')
                        $("#patch_img").val(samStrs.url + file);
                }
                else {
        ...
        $('#tabs').tabs({
            activate: function(event, ui) {
                ...
                if (el == 'tabs-2') {
                    if (rcctt.is(':visible') && w2ui['ctt-grid'])
                        cttGrid.w2render('ctt-grid');
                    if (rcxct.is(':visible') && w2ui['x-ctt-grid'])
                        xcttGrid.w2render('x-ctt-grid');
                    if (rcxid.is(':visible') && w2ui['x-posts-grid'])
                        xpostsGrid.w2render('x-posts-grid');
                    if (rcac.is(':visible') && w2ui['cats-grid'])
                        catsGrid.w2render('cats-grid');
                    if (rcxc.is(':visible') && w2ui['x-cats-grid'])
                        xcatsGrid.w2render('x-cats-grid');
                    if (rcau.is(':visible') && w2ui['auth-grid'])
                        authGrid.w2render('auth-grid');
                    if (rcxa.is(':visible') && w2ui['x-auth-grid'])
                        xauthGrid.w2render('x-auth-grid');
                    if (rcat.is(':visible') && w2ui['tags-grid'])
                        tagsGrid.w2render('tags-grid');
                    if (rcxt.is(':visible') && w2ui['x-tags-grid'])
                        xtagsGrid.w2render('x-tags-grid');
                    if (rccu.is(':visible') && w2ui['cust-grid'])
                        custGrid.w2render('cust-grid');
                    if (rcxu.is(':visible') && w2ui['xcust-grid'])
                        xcustGrid.w2render('x-cust-grid');
                }
                if (el == 'tabs-3')
                    if (xViewUsers.is(':visible') && w2ui['users-grid'])
        ...
        $("input:radio[name=view_type]").click(function() {
            ...
            switch (cval) {
                case '0':
                    if (rcvt0.is(':hidden'))
                        rcvt0.show('blind', {direction: 'vertical'}, 500);
                    if (rcvt2.is(':visible'))
                        rcvt2.hide('blind', {direction: 'vertical'}, 500);
                    xId.attr('disabled', false);
                    break;
                case '1':
                    if (rcvt0.is(':visible'))
                        rcvt0.hide('blind', {direction: 'vertical'}, 500);
                    if (rcvt2.is(':visible'))
                        rcvt2.hide('blind', {direction: 'vertical'}, 500);
                    xId.attr('disabled', false);
                    break;
        ...
        $("input:radio[name=ad_users]").click(function() {
            ...
            if (uval == '0') {
                if (custUsers.is(':visible'))
                    custUsers.hide('blind', {direction: 'vertical'}, 500);
            }
            else {
        ...
        adUsersReg.click(function() {
            if (adUsersReg.is(':checked'))
                ...
                xRegUsers.hide('blind', {direction: 'vertical'}, 500);
        });
        ...
        xAdUsers.click(function() {
            if (xAdUsers.is(':checked'))
                ...
                xViewUsers.hide('blind', {direction: 'vertical'}, 500);
        });

        $('#ad_swf').click(function() {
            if ($('#ad_swf').is(':checked'))
                $('#swf-params').show('blind', {direction: 'vertical'}, 500);
            else
                $('#swf-params').hide('blind', {direction: 'vertical'}, 500);
        });
        ...
        $('#ad_schedule').click(function() {
            if ($('#ad_schedule').is(':checked'))
                $('#rc-sc').show('blind', {direction: 'vertical'}, 500);
            else
                $('#rc-sc').hide('blind', {direction: 'vertical'}, 500);
        });

        $('#limit_hits').click(function() {
            if ($('#limit_hits').is(':checked'))
                $('#rc-hl').show('blind', {direction: 'vertical'}, 500);
            else
                $('#rc-hl').hide('blind', {direction: 'vertical'}, 500);
        });

        $('#limit_clicks').click(function() {
            if ($('#limit_clicks').is(':checked'))
                $('#rc-cl').show('blind', {direction: 'vertical'}, 500);
            else
                $('#rc-cl').hide('blind', {direction: 'vertical'}, 500);
        });
