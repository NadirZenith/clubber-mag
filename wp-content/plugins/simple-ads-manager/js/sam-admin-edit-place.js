...
(function($) {
    ...
    sam.media = media = {
        ...
        handleMediaAttachment: function(a) {
            ...
            if ('' == $(this.adName).val() && '' != attechment.title)
                $(this.adName).val(attechment.title);
            if ('' == $(this.adDesc).val() && '' != attechment.caption)
                $(this.adDesc).val(attechment.caption);
            if ('' == $(this.adAlt).val() && '' != attechment.alt)
                $(this.adAlt).val(attechment.alt);
        }
    };
    ...
    $(document).ready(function() {
        ...
        fu = new AjaxUpload(btnUpload, {
            ...
            onComplete: function(file, response) {
                ...
                if (response == "success") {
                    ...
                    if (em == 'item')
                        $("#ad_img").val(options.url + file);
                    else if (em == 'place')
                        $("#patch_img").val(options.url + file);
                }
                else {
        ...
        $('#patch_source_image').click(function() {
            if (rcpsi.is(':hidden'))
                rcpsi.show('blind', {direction: 'vertical'}, 500);
            if (rcpsc.is(':visible'))
                rcpsc.hide('blind', {direction: 'vertical'}, 500);
            if (rcpsd.is(':visible'))
                rcpsd.hide('blind', {direction: 'vertical'}, 500);
        });

        $('#patch_source_code').click(function() {
            if (rcpsi.is(':visible'))
                rcpsi.hide('blind', {direction: 'vertical'}, 500);
            if (rcpsc.is(':hidden'))
                rcpsc.show('blind', {direction: 'vertical'}, 500);
            if (rcpsd.is(':visible'))
                rcpsd.hide('blind', {direction: 'vertical'}, 500);
        });
        ...
        $('#patch_source_dfp').click(function() {
            ...
            if (rcpsc.is(':visible'))
                rcpsc.hide('blind', {direction: 'vertical'}, 500);
            if (rcpsd.is(':hidden'))
                rcpsd.show('blind', {direction: 'vertical'}, 500);
        });
