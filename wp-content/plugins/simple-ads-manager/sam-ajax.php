      ...
      switch ($action) {
            case 'sam_ajax_sam_click':
                  ...
                  if ($id > 0) {
                        ...
                        if ($result === 1) {
                              ...
                              echo json_encode(array('success' => false, 'id' => $id, 'sql' => $wpdb->last_query, 'error' => $wpdb->last_error));
                  } else
                        echo json_encode(array('success' => false, 'id' => $id));
                  break;
            ...
            case 'sam_ajax_sam_hit':
                  if (isset($_POST['id']) && isset($_POST['pid'])) {
                        ...
                        if (!empty($sql))
                              $result = $wpdb->query($wpdb->prepare($sql, $id, $pid));
                        if ($result === 1)
                              echo json_encode(array('success' => true, 'id' => $id, 'pid' => $pid));
                        else
