...
function facebookall_render_facebook_button() {
        ...
        if (!empty($fball_settings ['custom_button'])) {
                $fball_button .= '<a href="javascript:void(0);" title="Entra con Facebook" onclick="FbAll.facebookLogin();" class="fball_login_facebook"><img src="' . $fball_settings ['custom_button'] . '" style="cursor:pointer;"></a>';
        } else {
                $fball_button .= '<span id="fball-facebook-login">
        <a href="javascript:void(0);" title="Entra con Facebook" onclick="FbAll.facebookLogin();" class="fball_login_facebook"><span>' . $fball_settings ['fbicon_text'] . '</span></a></span>';
        }
        $fball_button .= '</div>
...
function facebookall_render_widget_button() {
        ...
        if (is_user_logged_in()) {
                ...
                echo '<br/><a href="' . wp_logout_url() . '">';
                _e('Log Out', 'facebookall');
...
function facebookall_render_fanbox() {
        $fball_settings = get_option('fball_settings');
        ?>
        <iframe src="//www.facebook.com/plugins/likebox.php?href=<?php echo urlencode($fball_settings['fanbox_pageurl']); ?>&amp;width=<?php echo $fball_settings['fanbox_width']; ?>&amp;height=<?php echo $fball_settings['fanbox_height']; ?>&amp;colorscheme=<?php if ($fball_settings['fanbox_color'] == '1') {
                echo 'light';
        } else {
                echo 'dark';
        } ?>&amp;show_faces=<?php if ($fball_settings['fanbox_faces'] == '0') {
                echo 'false';
        } else {
                echo 'true';
        } ?>&amp;stream=<?php if ($fball_settings['fanbox_stram'] == '0') {
                echo 'false';
        } else {
                echo 'true';
        } ?>&amp;header=<?php if ($fball_settings['fanbox_header'] == '0') {
                echo 'false';
        } else {
                echo 'true';
        } ?>" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:<?php echo $fball_settings['fanbox_width']; ?>px; height:<?php echo $fball_settings['fanbox_height']; ?>px;" allowTransparency="true"></iframe>
<?php
}
...
function facebookall_render_facepile() {
        $fball_settings = get_option('fball_settings');
        ?>
        <div class="fb-facepile" data-href="<?php echo trim($fball_settings['facepile_pageurl']) ?>" data-width="<?php echo $fball_settings['facepile_width'] ?>" data-max-rows="<?php echo $fball_settings['facepile_numrows'] ?>" data-colorscheme="<?php if ($fball_settings['facepile_color'] == '1') {
                echo 'light';
        } else {
                echo 'dark';
        } ?>" data-size="<?php if ($fball_settings['facepile_size'] == '0') {
                echo 'small';
        } elseif ($fball_settings['facepile_size'] == '1') {
                echo 'medium';
        } else {
                echo 'large';
